# File: src/cell_decomposition/cell_segmentation.py

from typing import List, Dict, Union
import Rhino.Geometry as rg
from src.cell_decomposition.cell_types import (
...
def _calculate_corner_points(u_start: float, u_end: float, v_start: float, v_end: float, base_plane) -> List:
    """
...
def decompose_wall_to_cells(
    ...
    wall_boundary_cell_data = create_wall_boundary_cell_data(u_range=[0.0, wall_length], v_range=[0.0, wall_height])
    
    # 2. For each opening, create an opening cell
    opening_cells_data = []
    for opening_data in opening_data_list:
        oc_data = create_opening_cell_data(
            u_range=[opening_data["start_u_coordinate"],
                     opening_data["start_u_coordinate"] + opening_data["rough_width"]],
            v_range=[opening_data["base_elevation_relative_to_wall_base"],
    ...
    sorted_opening_cells_data = sorted(opening_cells_data, key=lambda cell_data: cell_data["u_start"])
    ...
    for opening_cell_data in opening_cells_data:
        ...
        if (u_end - u_start) > tol and (v_start > tol):
            ...
            print("Skipping sill cripple cell: u_range=({},{}) or insufficient v_range (v_start={})"
                  .format(u_start, u_end, v_start))
        ...
        if (u_end - u_start) > tol and ((wall_height - opening_cell_data["v_end"]) > tol):
            ...
            print("Skipping header cripple cell: insufficient vertical space (v_end={}, wall_height={})"
                  .format(opening_cell_data.get("v_end"), wall_height))
        ...
        if isinstance(cell_group, list):
            for cell in cell_group:
                ...
                if None not in [u_s, u_e, v_s, v_e]:
                    cell["corner_points"] = _calculate_corner_points(u_s, u_e, v_s, v_e, base_plane)
            ...
            if None not in [u_s, u_e, v_s, v_e]:
                cell_group["corner_points"] = _calculate_corner_points(u_s, u_e, v_s, v_e, base_plane)
