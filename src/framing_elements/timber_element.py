# File: src/framing_elements/timber_element.py
...
def _get_point_at_uv(cell_data: Dict, u_fraction: float, v_fraction: float) -> rg.Point3d:
...
# --- Stud Creation ---
def create_stud(cell_data: Dict, stud_width: Optional[float] = None, stud_depth: Optional[float] = None, profile: Optional[rg.Rectangle3d] = None) -> rg.Curve:
    ...
    if profile:
        ...
        stud_width = stud_width if stud_width is not None else FRAMING_PARAMS["stud_width"]
        stud_depth = stud_depth if stud_depth is not None else FRAMING_PARAMS["stud_depth"]
...
def create_sill_cripple(cell_data: Dict, stud_width: Optional[float] = None, stud_depth: Optional[float] = None, profile: Optional[rg.Rectangle3d] = None) -> rg.Curve:
    """Creates a sill cripple (centerline) from a Sill Cripple Cell (SCC)."""
    return create_stud(cell_data, stud_width, stud_depth, profile)  # Reuse stud creation

# --- Header Cripple Creation ---
def create_header_cripple(cell_data: Dict, stud_width: Optional[float] = None, stud_depth: Optional[float] = None, profile: Optional[rg.Rectangle3d] = None) -> rg.Curve:
...
# --- King Stud Creation ---
def create_king_stud(opening_cell_data: Dict, wall_data: Dict, stud_width: Optional[float] = None, stud_depth: Optional[float] = None, profile: Optional[rg.Rectangle3d] = None) -> List[rg.Curve]:
    ...
    # Left King Stud
    u_left = opening_cell_data["u_start"] - FRAMING_PARAMS["trimmer_offset"] - FRAMING_PARAMS["king_stud_offset"] - stud_width # Offset to the *left*
    v_start = opening_cell_data["v_start"]
    v_end = opening_cell_data["v_end"]

    pt_start_left = rg.Point3d(base_plane.Origin.X + u_left, base_plane.Origin.Y, base_plane.Origin.Z + v_start)
    pt_end_left = rg.Point3d(base_plane.Origin.X + u_left, base_plane.Origin.Y, base_plane.Origin.Z + v_end)
    king_studs.append(_create_line_curve(pt_start_left, pt_end_left))


    # Right King Stud
    u_right = opening_cell_data["u_end"] + FRAMING_PARAMS["trimmer_offset"] + FRAMING_PARAMS["king_stud_offset"]  # Offset to the *right*
    pt_start_right = rg.Point3d(base_plane.Origin.X + u_right, base_plane.Origin.Y, base_plane.Origin.Z + v_start)
    pt_end_right = rg.Point3d(base_plane.Origin.X + u_right, base_plane.Origin.Y, base_plane.Origin.Z + v_end)
    king_studs.append(_create_line_curve(pt_start_right, pt_end_right))

    return king_studs

# --- Trimmer Stud Creation ---
def create_trimmer_stud(opening_cell_data: Dict, wall_data: Dict, stud_width: Optional[float] = None, stud_depth: Optional[float] = None, profile: Optional[rg.Rectangle3d] = None) -> List[rg.Curve]:
    ...
    # Left trimmer Stud
    u_left = opening_cell_data["u_start"] - FRAMING_PARAMS["trimmer_offset"] - stud_width / 2  # Offset to the *left*
    v_start = opening_cell_data["v_start"]
    v_end = opening_cell_data["v_end"]
    pt_start_left = rg.Point3d(base_plane.Origin.X + u_left, base_plane.Origin.Y, base_plane.Origin.Z + v_start)
    pt_end_left = rg.Point3d(base_plane.Origin.X + u_left, base_plane.Origin.Y, base_plane.Origin.Z + v_end)
    ...
    # Right trimmer Stud
    u_right = opening_cell_data["u_end"] + FRAMING_PARAMS["trimmer_offset"] + stud_width / 2  # Offset to the *right*
    pt_start_right = rg.Point3d(base_plane.Origin.X + u_right, base_plane.Origin.Y, base_plane.Origin.Z + v_start)
    pt_end_right = rg.Point3d(base_plane.Origin.X + u_right, base_plane.Origin.Y, base_plane.Origin.Z + v_end)
...
# --- Header Creation ---
def create_header(opening_data: Dict, wall_data: Dict, header_width: Optional[float] = None, header_depth: Optional[float] = None, profile: Optional[rg.Rectangle3d] = None) -> rg.Curve:
    """Creates a header (centerline) above an opening."""
    if profile:
        header_width = profile.Width
        header_depth = profile.Height
    else:
        header_width = header_width if header_width is not None else FRAMING_PARAMS["header_width"]
        header_depth = header_depth if header_depth is not None else FRAMING_PARAMS["header_depth"]

    base_plane = wall_data["base_plane"]
    start_u = opening_data["start_u_coordinate"]
    rough_width = opening_data["rough_width"]
    end_u = start_u + rough_width
    v_start = opening_data["base_elevation_relative_to_wall_base"] + opening_data["rough_height"]  # Top of opening

    # For now, simple header along the U direction at the opening's top.
    pt_start = rg.Point3d(base_plane.Origin.X + start_u, base_plane.Origin.Y, base_plane.Origin.Z + v_start)
    pt_end = rg.Point3d(base_plane.Origin.X + end_u, base_plane.Origin.Y, base_plane.Origin.Z + v_start)

    return _create_line_curve(pt_start, pt_end)

# --- Sill Creation ---
def create_sill(opening_data: Dict, wall_data: Dict, sill_width: Optional[float] = None, sill_depth: Optional[float] = None, profile: Optional[rg.Rectangle3d] = None) -> rg.Curve:
    """Creates a sill (centerline) below an opening."""
    if profile:
        sill_width = profile.Width
        sill_depth = profile.Height
    else:
        sill_width = sill_width if sill_width is not None else FRAMING_PARAMS["sill_width"]
        sill_depth = sill_depth if sill_depth is not None else FRAMING_PARAMS["sill_depth"]
...
# --- Top Plate Creation ---
def create_top_plate(wall_data: Dict, top_plate_layers: Optional[int] = None, plate_width: Optional[float] = None, plate_depth: Optional[float] = None, profile: Optional[rg.Rectangle3d] = None) -> List[rg.Curve]:
    """
    ...
    if profile:
        ...
        plate_width = plate_width if plate_width is not None else FRAMING_PARAMS["stud_width"]
        plate_depth = plate_depth if plate_depth is not None else FRAMING_PARAMS["plate_thickness"]

    top_plate_layers = top_plate_layers if top_plate_layers is not None else FRAMING_PARAMS["top_plate_layers"]
...
def create_bottom_plate(wall_data: Dict, bottom_plate_layers: Optional[int] = None, plate_width: Optional[float] = None, plate_depth: Optional[float] = None, profile: Optional[rg.Rectangle3d] = None) -> List[rg.Curve]:
    """
    ...
    bottom_plate_layers = bottom_plate_layers if bottom_plate_layers is not None else FRAMING_PARAMS["bottom_plate_layers"]
    base_curve = wall_data["wall_base_curve"]
