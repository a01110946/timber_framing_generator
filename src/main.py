...
if DEV_MODE:
	...
	from src.dev_utils.reload_modules import clear_module_cache, reload_project_modules
	
	# Clear cache and reload modules
	clear_module_cache(project_root)
	reload_project_modules(project_root)

# Now import using relative paths from within src
from src.wall_data.wall_selector import pick_walls_from_active_view
from src.wall_data.revit_data_extractor import extract_wall_data_from_revit
from src.utils.data_extractor import (
	extract_wall_assembly_keys,
	convert_all_walls_data_to_nested_lists
)
from src.cell_decomposition.cell_visualizer import create_rectangles_from_cell_data
from src.framing_elements.plates import create_plates
from tests.framing_elements.test_plates import TestPlateSystem
from src.framing_elements.studs import calculate_stud_locations, generate_stud
...
if run:
	...
	for wall_index, wall_data in enumerate(wall_dict):
		try:
			...
			test_results[str(wall_index)] = {
				'plates': bottom_plates + top_plates
			}
	...
	solids = plate_tester.get_visualization_geometry(
		geometry_type="platform_geometry", 
		platform="rhino"
	)
	...
	opening_location_point = th.list_to_tree(nested_data["opening_location_point"], source=[0])
	...
	base_elevation_relative_to_wall_base = th.list_to_tree(nested_data["base_elevation_relative_to_wall_base"], source=[0])
