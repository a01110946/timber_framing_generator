...
from timber_framing_generator.utils.units import ProjectUnits, convert_to_feet, convert_from_feet, get_project_units
...
class WallAssembly:
    ...
    def total_wall_thickness(self) -> float:
        ...
        return (self.exterior_layer_thickness +
...
def get_assembly_dimensions(units: ProjectUnits = None) -> Dict[str, float]:
    ...
    return {
        "exterior_layer": convert_from_feet(WALL_ASSEMBLY.exterior_layer_thickness, units),
        "core_layer": convert_from_feet(WALL_ASSEMBLY.core_layer_thickness, units),
        "interior_layer": convert_from_feet(WALL_ASSEMBLY.interior_layer_thickness, units),
