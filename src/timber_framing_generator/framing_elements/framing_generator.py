...
from timber_framing_generator.framing_elements.header_cripples import HeaderCrippleGenerator
    ...
    def _generate_studs(self) -> None:
        ...
        try:
            ...
            stud_generator = StudGenerator(
                ...
                self.framing_elements.get('king_studs', []) # Pass king studs to avoid overlap
            )
            ...
            # Collect debug geometry
            for key in ['points', 'planes', 'profiles', 'paths']:
                if key in stud_generator.debug_geometry and stud_generator.debug_geometry[key]:
                    if key in self.debug_geometry:
                        self.debug_geometry[key].extend(stud_generator.debug_geometry[key])
                    else:
