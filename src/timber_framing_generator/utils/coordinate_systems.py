# File: timber_framing_generator/utils/coordinate_systems.py
...
class WallCoordinateSystem:
    ...
    def __init__(self, wall_data: Dict[str, Any]):
        ...
        self.debug_geometry = {"axes": [], "grid": [], "origin": None}
        ...
        try:
            ...
            try:
                ...
                if (
                    self._to_world_transform is None
                    or not self._to_world_transform.IsValid
                ):
                ...
                if (
                    self._to_wall_transform is None
                    or not self._to_wall_transform.IsValid
                ):
    ...
    def is_valid(self) -> bool:
        ...
        if not (
            self.base_plane is not None
            and self.wall_curve is not None
            and self._to_world_transform is not None
            and self._to_wall_transform is not None
        ):
            return False

        # Check if transforms are valid
        if not (self._to_world_transform.IsValid and self._to_wall_transform.IsValid):
            return False
    ...
    def wall_to_world(self, u: float, v: float, w: float = 0.0) -> Optional[rg.Point3d]:
        ...
        try:
            ...
            transform_valid = (
                self._to_world_transform is not None
                and self._to_world_transform.IsValid
            )
            ...
            if self.base_plane is not None:
                potentially_unstable = (
                    abs(self.base_plane.XAxis.X) < 1e-10
                    or abs(self.base_plane.YAxis.Y) < 1e-10
                )
            ...
            if not transform_valid or potentially_unstable:
                if not transform_valid:
                    print(
                        f"Transform invalid for u={u}, v={v}, w={w} - using point_at method"
                    )
                elif potentially_unstable:
                    print(
                        f"Potentially unstable geometry for u={u}, v={v}, w={w} - using point_at method"
                    )

                direct_point = self.point_at(u, v, w)
                if direct_point is None:
                    print(
                        f"  point_at also failed - base_plane is {self.base_plane is not None}"
                    )
                return direct_point
            ...
            if not world_point.Transform(self._to_world_transform):
                print(
                    f"Point transformation failed for u={u}, v={v}, w={w} - falling back to point_at"
                )
                return self.point_at(
                    u, v, w
                )  # Fall back to direct method if transform fails
    ...
    def create_wall_plane(
        self, u: float, v: float, normal_dir: str = "w"
    ) -> Optional[rg.Plane]:
        """
    ...
    def _create_axes_visualization(self) -> List[rg.Curve]:
        ...
        try:
            ...
            if u_axis is not None:
                curves.append(u_axis)
            if v_axis is not None:
                curves.append(v_axis)
            if w_axis is not None:
                curves.append(w_axis)
    ...
    def _create_grid_visualization(
        self, u_divisions: int = 5, v_divisions: int = 3
    ) -> List[rg.Curve]:
        ...
        try:
            ...
            if wall_length <= 0 or wall_height <= 0:
                print(
                    f"Warning: Invalid wall dimensions for grid visualization. Length: {wall_length}, Height: {wall_height}"
                )
                return curves

            print(
                f"Creating grid visualization for wall: Length={wall_length}, Height={wall_height}"
            )
            ...
            for i in range(u_divisions + 1):
                ...
                try:
                    ...
                    if start is not None and end is not None:
                        ...
                        print(
                            f"  Warning: Invalid points for vertical grid line at u={u}"
                        )
                except Exception as e:
            ...
            for i in range(v_divisions + 1):
                ...
                try:
                    ...
                    if start is not None and end is not None:
                        ...
                        print(
                            f"  Warning: Invalid points for horizontal grid line at v={v}"
                        )
                except Exception as e:
...
class FramingElementCoordinates:
    ...
    def get_world_corners(self) -> List[Optional[rg.Point3d]]:
        ...
        try:
            ...
            corners = [
                self.coordinate_system.wall_to_world(
                    self.u_start, self.v_start, self.w_center
                ),
                self.coordinate_system.wall_to_world(
                    self.u_end, self.v_start, self.w_center
                ),
                self.coordinate_system.wall_to_world(
                    self.u_end, self.v_end, self.w_center
                ),
                self.coordinate_system.wall_to_world(
                    self.u_start, self.v_end, self.w_center
                ),
            ]
    ...
    def get_centerline(self) -> Optional[rg.Curve]:
        ...
        try:
            ...
            if start is None or end is None:
                print(
                    "Warning: Unable to create centerline - one or both points are None"
                )
                return None
        ...
        try:
            ...
            try:
                ...
                if corners and len(corners) == 4:
                    ...
                    print(
                        f"Warning: Invalid corners array: {len(corners) if corners else 0} points"
                    )
            except Exception as e:
            ...
            try:
                start_point = self.coordinate_system.wall_to_world(
                    self.u_start, self.v_start, self.w_center
                )
                end_point = self.coordinate_system.wall_to_world(
                    self.u_end, self.v_end, self.w_center
                )
