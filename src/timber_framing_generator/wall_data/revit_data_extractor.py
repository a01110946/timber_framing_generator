...
def extract_wall_data_from_revit(revit_wall: DB.Wall, doc) -> WallInputData:
    ...
    wall_top_elevation = (
        top_level.Elevation
        if top_level
        else wall_base_elevation
        + revit_wall.LookupParameter("Unconnected Height").AsDouble()
    ) + top_offset
    ...
    for insert_id in insert_ids:
        ...
        if isinstance(insert_element, DB.FamilyInstance):
            ...
            if rough_width_param and rough_height_param and sill_height_param:
                ...
                success, t = wall_base_curve_rhino.ClosestPoint(
                    opening_location_point_rhino
                )
